...
import { Hospital, Check, AlertCircle } from 'lucide-react';

const HospitalRecommendationCard = ({ candidate, isSelected, onSelect, isRecommended, selectionMode = false }) => {
  const {
    hospital_id,
    hospital_name,
    eligible,
  ...
  return (
    <div 
      ...
      onClick={() => eligible && selectionMode && onSelect(hospital_id)}
    >
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[10px] text-slate-400 mt-0.5">Emergency Capacity: {emergency_capacity}</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-3 gap-2 mt-3.5 border-t border-b border-navy-800 py-2.5 text-[11px] text-slate-400">
        ...
        <div>
          <span className="text-[9px] text-slate-500 uppercase tracking-wider block">Transport ETA</span>
          <span className="font-bold text-slate-200 font-mono">{estimated_transport_minutes.toFixed(1)} min</span>
        </div>
        <div>
          <span className="text-[9px] text-slate-500 uppercase tracking-wider block">Available Beds</span>
          <span className="font-bold text-slate-200 font-mono">{available_beds} beds</span>
        </div>
      </div>
      ...
      <div className="mt-3">
        {!eligible ? (
          <div className="flex items-start gap-1 text-[10px] text-red-400 bg-red-950/20 border border-red-500/10 p-2 rounded">
            ...
            <span>Not eligible: {reasons[0] || 'At Capacity'}</span>
          </div>
        ) : (
